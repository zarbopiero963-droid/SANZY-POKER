...
- [ ] **Follow-up PR2:** store anti-replay durevole (Postgres) al posto dell'in-memory effimero; proxy `/api` nel dev server Vite per il test full-stack locale (oggi via `pnpm build && pnpm start`).
