...
- [x] Migliorare la grafica 3D del tavolo di gioco → passaggio di design a step (`HANDOFF-DESIGN.md`), **una PR alla volta dopo merge del proprietario**. **Roadmap animazioni completata e confermata nel browser dal proprietario** (buon risultato):
  ...
  - [ ] (Follow-up opzionale, tracciato dallo Step 3) Timer/anello di turno **non-numerico** animato via `tick()`: un timer NUMERICO è stato rimandato perché a schermo cambierebbe ogni secondo → passerebbe dal render-gate e riattiverebbe lo sfarfallio; un **anello che si svuota** (senza numero) sarebbe gate-safe. Da decidere col proprietario: fare o abbandonare.
