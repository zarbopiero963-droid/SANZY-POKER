...
- [x] **Retention righe Postgres (#30, base):** cleanup **schedulato** (timer `setInterval` best-effort/`unref` ogni 6h in `server/index.ts`, con prime al boot) **+ opportunistico** a ogni `reserve` (DELETE righe > 30 giorni via `pruneExpired`) → la `password` non resta at-rest oltre la finestra nemmeno con traffico nullo, e l'anti-replay è una finestra scorrevole. Il ciclo di vita completo (scadenza legata alla sessione server-authoritative + `expiresAt`) resta al PR3 punto 2.
...
- [x] **Offuscamento bundle + pulizia tooling Manus (richiesta owner «nessuno deve vedere la meccanica»):** `vite.config.ts` offusca l'output di produzione con `vite-plugin-javascript-obfuscator` (solo il nostro codice, `exclude node_modules`; mangling + string array cifrato + control-flow flattening 0.5 + dead-code 0.2; `sourcemap:false`). Sorgente nel repo **lasciato leggibile** (offuscarlo romperebbe test/typecheck). Rimossi: `client/public/__manus__/debug-collector.js`, `client/src/components/ManusDialog.tsx` (codice morto), i plugin Vite Manus (debug-collector/runtime/storage-proxy), `jsxLocPlugin` (`@builder.io/vite-plugin-jsx-loc`, iniettava percorsi-sorgente nel DOM) e gli `allowedHosts .manus*`; deps `@builder.io/vite-plugin-jsx-loc` e `vite-plugin-manus-runtime` disinstallate. Motore invariato (272 test verdi). Verifica offuscamento = ispezione del `dist/` buildato (identificatori del motore assenti in chiaro).
- [x] **Lingua business: auto-rilevamento + selettore esplicito (fix owner):** la landing B2B auto-rileva la lingua del browser alla prima visita (`detectBizLocale`, pura e testata; italiano→it, altra→en; scelta salvata ha la precedenza) e il vecchio toggle «mostra la destinazione» (che sembrava invertito) è ora un **selettore a due pulsanti IT | EN** espliciti (`aria-pressed`, attivo evidenziato). Motore invariato; 278 test verdi (6 nuovi su `detectBizLocale`); smoke Playwright (IT→it, EN→en, FR→en, click EN→inglese). Lo StartScreen del gioco (4 flag) era già corretto.
- [x] **Correzione refuso nome proprietario «Zambo» → «Zarbo» + bump NDA_VERSION (fix owner):** corretto in `shared/ndaText.ts` (NDA IT+EN, fonte unica client+PDF), `server/nda/signing.ts` (autore PDF) e `landingI18n.ts` (footer/T&C/privacy IT+EN). `NDA_VERSION` 1.0→**1.1-clickwrap** (identità di parte = testo materiale, invariante «versione == testo»). Test di regressione hard (template e legali contengono «Piero Zarbo», mai «Zambo»); 280 test verdi; motore invariato.
- [ ] **Occultamento REALE della meccanica = lato server (segue #30 punto 2).** L'offuscamento client è un deterrente, non una garanzia: il codice è scaricato dal browser. Spostare regole/stato/bot sul server (client solo rendering) è l'unico modo per non farla vedere davvero; si allinea alla sessione server-authoritative del PR3 punto 2.
