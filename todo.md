...
- [x] **Copia dell'NDA firmato all'AZIENDA (spec #26):** `sendNdaEmail` invia, oltre alla notifica all'owner, una **copia** del PDF firmato all'email aziendale del firmatario (invio SEPARATO, così l'indirizzo dell'owner non è svelato). **Gated** su `NDA_COPY_TO_COMPANY=1` perché col mittente di prova `onboarding@resend.dev` Resend consegna solo all'account owner: l'owner accende il flag quando ha un **dominio verificato** (`RESEND_FROM`). **Best-effort**: una copia fallita NON invalida la firma né blocca la demo (l'artefatto autorevole è già arrivato all'owner). La risposta porta `companyCopySent`; la schermata di sblocco mostra «una copia dell'NDA firmato è stata inviata alla tua email aziendale: …» quando la copia è partita. Aggiunto anche il tag wordmark **[B2B DEMO]** (dai commenti #26). Test hard: gating off/on, invio separato, best-effort su fallimento, env-gating, propagazione `companyCopySent` client+server (mock di `resend`, nessuna rete).
- [ ] **Follow-up PR3 (server-authoritative pieno):** sessione/token firmato dal server + `expiresAt` verificato lato server (non fidarsi solo di `localStorage`); store durevole (Postgres) al posto dell'in-memory effimero, con idempotenza sulla risposta persa (retry recupera la firma esistente); font Unicode nel PDF (`@pdf-lib/fontkit`) per nomi non-latini; proxy `/api` nel dev server Vite per il test full-stack locale (oggi via `pnpm build && pnpm start`).
