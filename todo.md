...
- [ ] Migliorare la grafica 3D del tavolo di gioco → passaggio di design a step (`HANDOFF-DESIGN.md`), **una PR alla volta dopo merge del proprietario**:
  ...
  - [~] Step 4 — stabilità/performance, meno lavoro per-frame: `tick()` riassegna colore del bordo attivo e alpha dei `pulseDots` **solo quando il byte quantizzato (0..255) cambia** dal frame precedente, riducendo le invalidazioni GUI su mobile; la stringa colore è costruita dal byte cache-ato (`withAlphaByte`), così chiave e valore derivano dalla stessa fonte; matematica di pulsazione unificata in `anim.ts` (`pulse01`/`dotPulseAlpha`) e usata sia dai punti sia dal bordo; helper `registerActiveBorder()` che invalida la cache alla registrazione; guard su `NaN` (`Number.isNaN`) in `alphaByteHex` — mai `"NaN"` nel colore, mentre gli infiniti restano gestiti dal clamp. Solo rendering, motore invariato.
