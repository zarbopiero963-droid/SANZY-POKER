...
- [x] **Retention righe Postgres (#30, base):** cleanup **opportunistico** a ogni `reserve` (DELETE righe > 30 giorni) → la `password` non resta at-rest indefinitamente e l'anti-replay è una finestra scorrevole. Il ciclo di vita completo (scadenza legata alla sessione server-authoritative + `expiresAt`) resta al PR3 punto 2.
