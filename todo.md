...
- [ ] Migliorare la grafica 3D del tavolo di gioco → passaggio di design a step (`HANDOFF-DESIGN.md`), **una PR alla volta dopo merge del proprietario**:
  ...
  - [~] Step 3 — transizioni di fase/turno morbide: **bordo attivo pulsante** (il posto del giocatore di turno "respira" in arancione, animato nel loop `tick(elapsed)` via `client/src/game/anim.ts`, gate-safe) + **dissolvenza** delle carte appena scoperte (`visibility` 0.35→1 durante flip/slide in `createCard`, solo per le carte animate). Il **timer numerico è rimandato** perché a schermo cambierebbe ogni secondo → passerebbe dal render-gate e riattiverebbe lo sfarfallio/azzeramento slider (eventuale anello non-numerico via `tick` in un incremento successivo). Solo rendering, motore invariato.
    - Smoke checklist (browser, WebGL non testabile offline): (1) all'inizio del turno di un giocatore il suo bordo/anello pulsa in arancione e si spegne quando il turno passa; (2) allo scoprire flop/turn/river/Piatto 2 le nuove carte si dissolvono entrando (le carte già in tavola NON ri-dissolvono a ogni puntata) — controllare che fronte e dorso non risultino troppo trasparenti durante lo slide (la `visibility` in Babylon non si moltiplica parent→figli, ma va verificato a schermo); (3) durante il turno umano lo slider di rilancio resta stabile (nessun rebuild per-frame); (4) verifica desktop e mobile.
  - [~] Step 4 — stabilità/performance, meno lavoro per-frame: `tick()` riassegna il colore del bordo attivo **solo quando il valore quantizzato cambia** (cache `lastBorderColor`), riducendo le invalidazioni GUI su mobile; cache anche per i `pulseDots` (`lastDotPulse`, pulsazione quantizzata 0..255); matematica di pulsazione unificata in `anim.ts` (`pulse01`/`dotPulseAlpha`) e usata sia dai `pulseDots` sia dal bordo; helper `registerActiveBorder()` (invalida la cache) per i posti attivi; guard su `NaN` (`Number.isNaN`) in `alphaByteHex` — mai `"NaN"` nel colore, mentre gli infiniti restano gestiti dal clamp. Solo rendering, motore invariato.
    - Smoke checklist (browser, WebGL non profilabile offline): (1) il bordo del giocatore di turno pulsa fluido come prima (nessun "scatto" dovuto alla cache); (2) su mobile il tavolo resta fluido durante puntate e cambi di turno, senza sfarfallio; (3) i `pulseDots` di stato pulsano come prima (matematica invariata); (4) desktop + mobile.
