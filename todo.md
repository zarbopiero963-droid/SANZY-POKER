...
- [ ] Migliorare la grafica 3D del tavolo di gioco → passaggio di design a step (`HANDOFF-DESIGN.md`), **una PR alla volta dopo merge del proprietario**:
  ...
  - [~] Step 4 — stabilità/performance, meno lavoro per-frame: `tick()` riassegna il colore del bordo attivo **solo quando il valore quantizzato cambia** (cache `lastBorderColor`), riducendo le invalidazioni GUI su mobile; matematica di pulsazione unificata in `anim.ts` (`pulse01`/`dotPulseAlpha`) e usata sia dai `pulseDots` sia dal bordo; helper `registerActiveBorder()` per i posti attivi; guard `Number.isFinite` in `alphaByteHex` (mai `"NaN"` nel colore). Solo rendering, motore invariato.
