...
export function botDecision(
  table: TableState,
  playerIndex: number
): { action: PokerAction; raiseTo?: number } {
  ...
  const raiseTo = Math.min(
    stack,
    table.roundMaxBet + table.bigBlind * (2 + Math.floor(random * 3))
  );
  const canRaise =
    table.roundRaises === 0 &&
    raiseTo > table.roundMaxBet &&
    player.chips > callAmount + table.bigBlind;

  if (strength >= 5) {
    if (canCheck)
      return canRaise && random < 0.28 * player.aggression
        ? { action: "raise", raiseTo }
        : { action: "check" };
    if (canRaise && random < 0.16 * player.aggression)
      return { action: "raise", raiseTo };
    return { action: "call" };
  }
  if (strength >= 3) {
    if (canCheck)
      return canRaise && random < 0.22 * player.aggression
        ? { action: "raise", raiseTo }
        : { action: "check" };
    if (callAmount > stack * 0.6)
      return random < 0.5 ? { action: "call" } : { action: "fold" };
    return canRaise && random < 0.14 * player.aggression
      ? { action: "raise", raiseTo }
      : { action: "call" };
  }
  if (strength >= 1) {
